...
const Router = {
  ...
  async handleUserStatus(url, env) {
    ...
    try {
      ...
      const userJson = JSON.stringify({
        ...
        custom_configs: user.custom_configs
      });
  ...
  async handleApi(request, url, env, ctx) {
    ...
    if (url.pathname.startsWith('/api/users')) {
      ...
      if (isUserAction) {
        const isConfigsAction = pathParts[pathParts.length - 1] === 'configs';
        const username = decodeURIComponent(isConfigsAction ? pathParts[pathParts.length - 2] : pathParts.pop());
        ...
        if (request.method === 'POST' && url.pathname.endsWith('/configs')) {
          const body = await request.json();
          await env.DB.prepare("UPDATE users SET custom_configs = ? WHERE username = ?").bind(body.configs || null, username).run();
          return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
        }
      } else {
        if (request.method === 'GET') {
...
const DbService = {
  async ensureSchema(db) {
    ...
    try { await db.prepare("ALTER TABLE users ADD COLUMN custom_configs TEXT").run(); } catch (e) { }
...
const SubscriptionService = {
  ...
  async generateText(user, host) {
    if (user.custom_configs && user.custom_configs.trim() !== '') {
      const lines = user.custom_configs.split('\n').map(l => l.trim()).filter(Boolean);
      return new Response(btoa(unescape(encodeURIComponent(lines.join('\n')))), {
        headers: {
          "Content-Type": "text/plain; charset=utf-8",
          "Access-Control-Allow-Origin": "*",
          "Cache-Control": "no-store"
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    ...
    <script>
        ...
        function getVlessLink() {
            ...
            if (u.custom_configs && u.custom_configs.trim() !== '') {
                return u.custom_configs.trim();
            }
            const host = getHost();
            var ips = [host];
            if (u.ips) {
