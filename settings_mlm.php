...
<div class="flex gap-8" style="margin-bottom:16px;">
    ...
    <h2 style="margin:0;font-size:17px;"><?= icon('tune', 'icon-sm') ?> تنظیمات سراسری پنل MLM</h2>
</div>
<div class="text-sm muted" style="margin-bottom:14px;">
    برای مدیریت کاربران این پنل به <a href="<?= url('/mlm_users.php?account_id=' . (int) $accountId) ?>">صفحهٔ کاربران MLM</a> بروید.
</div>
...
<div class="card" style="border-color:var(--red-error);">
    ...
    <p class="text-sm muted" style="margin-top:4px;">حذف دیپلوی، Worker و دیتابیس MLM این اکانت را از Cloudflare پاک می‌کند و همهٔ کاربران/کانفیگ‌های ساخته‌شده در آن برای همیشه از بین می‌رود. بعد از حذف می‌توانید از داشبورد دوباره نصب کنید.</p>
    <button class="btn btn-ghost btn-sm" style="color:var(--red-error);margin-top:8px;" id="removeDeployBtn"><?= icon('delete') ?> حذف دیپلوی MLM</button>
</div>
...
document.getElementById('removeDeployBtn').addEventListener('click', function () {
    if (!confirm('دیپلوی MLM این اکانت به‌طور کامل حذف شود؟ این کار غیرقابل‌بازگشت است و همهٔ کاربران این پنل از بین می‌روند.')) return;
    CWP.runAction(this, '/api/remove_deploy_mlm.php', { account_id: ACCOUNT_ID }, { reload: false, onSuccess: function () { window.location.href = CWP.url('/dashboard.php'); } });
});
CWP.apiPost('/api/mlm_settings_fetch.php', { account_id: ACCOUNT_ID }).then(function (res) {
...
document.getElementById('settingsForm').addEventListener('submit', function (e) {
    e.preventDefault();
    CWP.runAction(document.getElementById('saveBtn'), '/api/mlm_settings_save.php', {
