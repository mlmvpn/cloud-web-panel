...
$result = cw_zeus_get_user_configs($account, $username);
...
foreach ($result['configs'] as $uri) {
    ...
    $nodes[] = [
        'name' => $fragment !== '' ? $fragment : ('Zeus ' . $username),
        'uri' => $uri,
        'type' => strpos($uri, 'vless') === 0 ? 'vless' : 'trojan',
        'engine_type' => 'ZEUS',
    ];
}
$groupId = create_cloud_group($userId, (int) $account['id'], 'ZEUS', 'کانفیگ‌های کاربر: ' . $username, $nodes);
